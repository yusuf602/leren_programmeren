...
class rooms:
    ...
    def room2(self):
        ...
        if self.route.get_previous_room() in ['4', '6', '9', '8']:
            ...
            if self.player.get_noice() > 1 or self.player.has_pickup('crystal'):
                ...
                elif self.player.has_pickup('sword'):
    ...
    def room8(self):
        ...
        if self.route.get_previous_room() == '5':
            ...
            self.story.write('Het water is helder en verfrissend.')

            answer = 'a'

            if self.story.write('Wil je er iets van drinken? (Y/N)', ['y', 'n']) == 'y':
                self.story.write('Terwijl je je handen vult met water en drinkt van de fontein, zweer je dat je een doorgang onderin het bassin ziet.')
    ...
    def room9(self):
        ...
        if lowerlevel:
            ...
            if answer == 'b':
                ...
                self.story.write('Dit zou wel eens een goede pikhouweel kunnen zijn om dat gat groter te maken...')
        ...
        if self.route.roomanswers.count(self.DEFAULT_ROOM_ANSWER) == 0:
            ...
            if self.route.get_previous_room() in ['2', '3', '6']:
                ...
                if answer != False:
                    ...
                    self.story.write('')
                    self.story.write('Een rood licht boven de deur begint te knipperen en de kamer begint zicht te vullen met gas.')

                    if self.player.has_pickup('lever'):
                        answer = True
                        self.story.write('Plots krijg je een goed idee, je hebt die hendel nog.')
                        self.story.write('Je zet hem tussen de deur en wringt de deur open.')
                        self.story.write('"Zo eindelijk vrij", denk je en je stapt naar buiten.')
